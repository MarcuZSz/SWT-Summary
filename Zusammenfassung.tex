...
\begin{itemize}
\item $\overline{\text{Lieferrate}} = \frac{\overline{\text{WIP}}}{\overline{\text{Durchlaufzeit}}}$
\item alternativ: $\overline{\text{Durchsatz}} = \frac{\overline{\text{WIP}}}{\overline{\text{TIP}}}$
...
\subsection{Funktionale \& nichtfunktionale Anforderungen}

\subsubsection{Funktionale Anforderungen}

\textbf{Beschreiben was das System tun soll:}
\begin{itemize}
\item Reaktion auf Eingaben
\item Verhalten in bestimmten Situationen
\item eventuell auch, was das System nicht tun soll
\item reichen von allgemeinen bis zu sehr detaillierten technischen Anforderungen
\item müssen vollständig \& konsistent sein, in der Praxis aber aufgrund vieler Stakeholder häufig nicht voll erreichbar
\end{itemize}

\noindent
\textbf{Beispiele (Mentcare-System):}
\begin{itemize}
\item Listen von Terminen durchsuchen
\item Tageslisten erzeugen
\item Eindeutige Identifikation der Mitarbeiter
\end{itemize}

\noindent
\textbf{Probleme:}
\begin{itemize}
\item Mehrdeutigkeit $\rightarrow$ führt zu Fehlinterpretationen
\item Domänenwissen kann fehlen (fachliche Anforderungen)
\end{itemize}

\subsubsection{Nichtfunktionale Anforderungen}

Beschreiben wie gut oder unter welchen Randbedingungen das System funktionieren soll.

\begin{figure}[H]
\begin{minipage}[t]{.45\textwidth}
\textbf{Arten nach Abb. 31:}
\begin{enumerate}
\item Produktanforderungen \\
(z. B. Performance, Zuverlässigkeit, Sicherheit)
\item Unternehmensanforderungen \\
(technische Standards, Entwicklungsprozesse)
\item Externe Anforderungen \\
(Gesetze, Normen, regulatorische Vorgaben)
\end{enumerate}
\end{minipage}
\hfill
\begin{minipage}[t]{.5\textwidth}
\centering
\vspace{-5mm}
\includegraphics[width=\linewidth]{pictures/netfunc.png}
\caption{Arten nichtfunktionaler Anforderungen}
\end{minipage}
\end{figure}

\noindent
\textbf{Beispiele (Mentcare):}
\begin{itemize}
\item Systemverfügbarkeit während Arbeitszeiten (Produktanforderung)
\item Authentifizierung über Mitarbeiterausweise (Unternehmensanforderung)
\item Einhaltung nationaler Datenschutzstandards (externe Anforderungen)
\end{itemize}

\newpage
\noindent
\textbf{Typisches Problem} \\
Kunden formulieren oft nur vage Ziele („einfach zu bedienen“). \\
$\rightarrow$ müssen in testbare Metriken überführt werden, z. B.:
\begin{itemize}
\item Anzahl der Nutzerfehler pro Stunde
\item Antwortzeiten
\item Verfügbarkeit in Sekunden pro Tag
\end{itemize}
\noindent
Nichtfunktionale Anforderungen sind häufig kritischer als funktionale, wenn sie verletzt werden, kann das System unbrauchbar werden.

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/metriknetfunc.png}
\caption{Metriken zur Festlegung nichtfunktionaler Anforderungen}
\end{figure}

\subsection{Prozesse des Requirements-Engineering}

\begin{figure}[H]
\begin{minipage}[t]{.45\textwidth}
\vspace{-50mm}
Das RE gliedert sich in drei Hauptaktivitäten:
\begin{enumerate}
\item Erhebung und Analyse
\item Spezifikation
\item Validierung
\end{enumerate}
\end{minipage}
...
\begin{minipage}[t]{.5\textwidth}
\centering
\includegraphics[width=\linewidth]{pictures/spiralrep.png}
\caption{Spiralmodell des Requirements-Engineering-Prozesses}
\end{minipage}
\end{figure}
\noindent
Diese Prozesse sind iterativ, nicht strikt linear. Abb. 33 zeigt ein Spiralmodell, bei dem Anforderungen schrittweise verfeinert werden. In frühen Iterationen: Fokus auf Geschäftsziele \& nichtfunktionale Anforderungen, später: detaillierte Systemanforderungen. \\
\textbf{Wichtig: Anforderungen ändern sich im Projektverlauf $\rightarrow$ Anforderungsmanagement notwendig}

\subsection{Anforderungserhebung \& -analyse}

Ziel: Verstehen, wie Stakeholder arbeiten und wie ein System diese Arbeit unterstützen soll.
\\
\noindent
\textbf{Herausforderungen:}
\begin{enumerate}
\item Stakeholder wissen oft nicht klar, was sie wollen
\item Sie verwenden domänenspezifischen Jargon
\item Unterschiedliche Stakeholder haben widersprüchliche Anforderungen
\item Politische Interessen beeinflussen Anforderungen
\item Geschäftsprozesse ändern sich während der Analyse
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/ablauf.png}
\caption{Prozess der Anforderungserhebung \& -analyse}
\end{figure}
\noindent
Dieser Prozess läuft wiederholt ab, bis die Gesamtspezifikation ausreichend stabil ist.
\\
\\
\textbf{Viewpoints}\\
Gruppierung der Anforderungen nach Stakeholder-Perspektiven, um Komplexität zu reduzieren.

\subsubsection{Techniken der Anforderungserhebung}

\begin{enumerate}
\item \textbf{Interview}
\begin{itemize}
\item häufigste Methode
\item offen, geschlossen oder gemischt
\item gut, um Probleme und Arbeitsweisen zu verstehen
\item aber: Gefahr von Missverständnissen, implizitem Wissen, politischem Schweigen
\item Tipps: keine vorgefassten Meinungen \&
konkrete Einstiegsfragen statt „Was wünschen Sie sich?“
\end{itemize}
\newpage
\item \textbf{Beobachtung/Ethnografie}
\begin{itemize}
\item Analyse der tatsächlichen Arbeitspraktiken
\item deckt implizite Anforderungen \& soziale/organisatorische Faktoren auf
\item besonders nützlich für kooperative Arbeitssituationen
\item kombiniert gut mit Prototyping
\end{itemize}
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[scale=.7]{pictures/techanf.png}
\caption{Ethnografie \& Herstellung von Prototypen für die Anforderungsanalyse}
\end{figure}

\subsubsection{User-Storys \& Szenarios}

Beide dienen dazu, Anforderungen anhand von konkreten Beispielen zu erfassen.
\\
\\
\textbf{User-Storys}
\begin{itemize}
\item narrativ, abstrakt, leicht verständlich
\item gut für frühe Diskussionen \\
Bsp: Story zum iLearn-System (Foto-Sharing im Unterricht)
\end{itemize}
\noindent
\textbf{Szenarios}
\begin{itemize}
\item detaillierter und strukturierter
\item beschreiben konkreten Ablauf inkl. Eingaben, Ausnahmen, paralleler Prozesse
\item Ausgangspunkt zur Herleitung präziser Anforderungen \\
Bsp: Szenario zum Hochladen und Moderieren von Fotos
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[scale=.2]{pictures/uml1.png}
\caption{Beispieldiagramm Use Case}
\end{figure}

\subsection{Aktivitätsdigramme}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{pictures/uml2.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{pictures/uml3.png}
\caption{Beispiel Sequenzdiagramm}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{pictures/uml4.png}
\caption{Beispiel Zustandsdiagramm}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{pictures/uml5.png}
\caption{Beispiel Klassendiagramm}
\end{figure}

\section{Software-Architektur}

\subsection{Architektonische Entwurfsentscheidungen}

\textbf{erfüllt funktionale \& nichtfunktionale Systemanforderungen}

\begin{enumerate}
\item kein fester Prozess, sondern Reihe zentraler Entscheidungen
\begin{itemize}
\item Wahl einer geeigneten Anwendungs- oder Referenzarchitektur
\item Entscheidung über Verteilung (zentral vs. verteilt)
\item Auswahl eines Architekturmusters (z. B. Schichten, Client-Server)
\item Zerlegung des Systems in Komponenten \& Subkomponenten
\item Steuerungsmodell (z. B. ereignis- oder aufrufgesteuert)
\item Berücksichtigung nichtfunktionaler Anforderungen
\end{itemize}
\item nichfunktionale Anforderungen mit großem Einfluss
\begin{itemize}
\item Performance: wenige, größere Komponenten
\item Sicherheit: Schichtenarchitektur
\item Zuverlässigkeit \& Verfügbarkeit: Redundanz
\item Wartbarkeit: kleine, unabhängige Komponenten
\end{itemize}
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/entarch.png}
\caption{Entwurfsentscheidungen zur Architektur}
\end{figure}
...
\textbf{nach dem 4+1-Sichtenmodel von Kruchten}
\begin{itemize}
\item Logische Sicht: zentrale Abstraktionen \& Klassen
\item Prozesssicht: Laufzeitprozesse \& Interaktion
\item Entwicklungssicht: Struktur für Entwicklerteams
\item Physische Sicht: Hardware \& Verteilung
\item Szenarien zur Verbindung der Sichten
\end{itemize}
\end{minipage}
\hfill
...
\centering
\vspace{-10mm}
\includegraphics[scale=.4]{pictures/archsicht.png}
\caption{Architektursichten}
\end{minipage}
\end{figure}
...
\begin{itemize}
\item beschreibt Struktur eines Systems
\item zeigt wann sinnvoll einsetzbar
\item typische Einsatzbereiche
\item Vor- \& Nachteile
\end{itemize}
...
\begin{minipage}[t]{.5\textwidth}
\centering
\includegraphics[scale=.3]{pictures/mvc1.png}
\caption{Struktur von MVC}
\end{minipage}
\hfill
\begin{minipage}[t]{.5\textwidth}
\centering
\includegraphics[scale=.25]{pictures/mvc2.png}
\caption{Architektur einer Webanwendung mit MVC-Muster}
\end{minipage}
\end{figure}
...
\begin{itemize}
\item System in hierarchische Schichten gegliedert
\item jede Schicht nutzt darunterliegende
\item Einsatz: neue Funktionen für vorhandenes System, mehrere Teams parallel, mehrstufige Sicherheit
\item Vorteile: gute Wartbarkeit, Austauschbarkeit, Portierbarkeit
\item Nachteile: Performanceprobleme, saubere Trennung oft schwierig
\end{itemize}

\begin{figure}[H]
\begin{minipage}[t]{.5\textwidth}
\centering
\includegraphics[scale=.3]{pictures/schichtarch1.png}
\caption{allgemeine Schichtarchitektur}
\end{minipage}
\hfill
\begin{minipage}[t]{.5\textwidth}
\centering
\includegraphics[scale=.25]{pictures/schichtarch2.png}
\caption{Architektur iLearn-System}
\end{minipage}
\end{figure}

\subsubsection{Repository-Architektur}

\begin{itemize}
\item zentrale Datenbasis, auf die alle Komponenten zugreifen
\item Komponenten kommnunizieren nicht direkt miteinander
\item Einsatz: große Datanmengen, langfristige Speicherung; z.B. Management-Informationssysteme, CAD-Systeme, IDEs
\item Vorteile: konsistente Daten, lose Kopplung
\item Nachteile: Single Point of Failure, Skalierungsprobleme
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/repoarch.png}
\caption{IDE Repo-Architektur}
\end{figure}

\subsubsection{Client-Server-Architektur}

\begin{itemize}
\item typisch für verteilte Systeme
\item Kommunikation über eine Netzwerk (z.B: HTTP)
\item Einsatz: gemeinsamer Datenzugriff von vielen Orten, variable Auslastung, Webanwendungen, Netzwerksysteme
\item Vorteile: gute Skalierbarkeit, zentrale Dienste
\item Nachteile: Serverausfall kritisch, netzabhängige Performance
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/csa.png}
\caption{Client-Server-Architektur einer Filmbibliothek}
\end{figure}

\subsubsection{Pipes-\&-Filter-Architektur}

\begin{itemize}
\item Daten fließen durch eine Kette von Verarbeitungsschritten
\item Jeder Filter transformiert die Daten
\item Einsatz: Datenverarbeitungssysteme, Stapelverarbeitung, geringe Benutzerinteraktion; z.B. Abrechnungssysteme, Compiler
\item Vorteile: einfache Erweiterung, Wiederverwendbarkeit
\item Nachteile: starre Datenformate, ungeeignet für interaktive Systeme
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/pfa.png}
\caption{Bsp. Pipes-\&-Filter-Architektur}
\end{figure}

\section{Design Patterns}

\subsection{Was ist eine Design Pattern?}

bewährtes Lösungsmuster für wiederkehrende Designprobleme in der Softwareentwicklung

\begin{enumerate}
\item Pattern-Name: prägnanter Name zur Referenzierung \& für bessere Kommunikation zw. Entwicklern
\item Problemstellung: Situation, in der das Pattern anwendbar ist, inklusive Kontext \& Bedingungen
\item Lösung: abstrakte Beschreibung der Anordnung von Klassen \& Objekten, keine konkrete Implementierung
\item Konsequenzen: Auswirkungen \& Kompromisse bei der Anwendung
\end{enumerate}

\subsection{Beschreibung der Design Patterns}

\begin{itemize}
\item \textbf{Pattern-Name \& Klassifizierung} \\
Bezeichnung \& Kategorisierung des Patterns
\item \textbf{Zweck} \\
Was bewirkt das Pattern und auf welche Designfragen antwortet es
\item \textbf{Auch bekannt als} \\
Alternative Bezeichnungen
\item \textbf{Motivation} \\
Beispielszenario, das die Problemlösung veranschaulicht
\item \textbf{Anwendbarkeit} \\
Situationen, in denen das Pattern sinnvoll einsetzbar ist
\item \textbf{Struktur} \\
Grafische Darstellung der Klassen \& Objekte (OMT-Notation)
\item \textbf{Teilnehmer} \\
Beteiligte Klassen/Objekte \& ihre Zuständigkeiten
\item \textbf{Interaktionen} \\
Art der Zusammenarbeit zw. den Teilnehmern
\item \textbf{Konsequenzen} \\
Vor- \& Nachteile, Auswirkungen auf die Systemstruktur
\item \textbf{Implementierung} \\
Tipps, Techniken, sprachspezifische Überlegungen
\item \textbf{Codebeispiele} \\
Implementierungen in C oder Smalltalk
\item \textbf{Praxisbeispiele} \\
min. zwei reale Anwendungsbeispiele
\item \textbf{Verwandte Patterns} \\
Beziehungen zu anderen Patterns
\end{itemize}

\subsection{Der Desgin-Patterns-Katalog}

enthält 23 Design Patterns, gruppiert nach Zweck \& Gültigkeitsbereich:
\begin{itemize}
\item Erzeugungsmuster (Creational): Abstract Factory, Builder, Factory Method, Prototype, Singleton
\item Strukturmuster (Structural): Adapter, Bridge, Composite, Decorator, Facade, Flyweight, Proxy
\item Verhaltensmuster (Behavioral): Chain of Responsibility, Command, Interpreter, Iterator, Mediator, Memento, Observer, State, Strategy, Template Method, Visitor
\end{itemize}

\subsection{Aufbau des Katalogs}

\begin{enumerate}
\item Zweck (was das Pattern bewirkt):
\begin{itemize}
\item Erzeugungsmuster: Objekterzeugungsprozess
\item Strukturmuster: Zusammensetzung von Klassen \& Objekten
\item Verhaltensmuster: Interaktion von Klassen/Objekten \& Verteilung der Zuständigkeiten
\end{itemize}
\item Gültigkeitsbereich (wo das Pattern wirkt):
\begin{itemize}
\item Klassenbasiert: Beeinflussen Vererbungsbeziehungen (statisch beim Kompilieren)
\item Objektbasiert: Beeinflussen Objektbeziehungen (dynamisch zur Laufzeit)
\end{itemize}
\item Zusätzliche Klassifizierung nach verwandtschaftlichen Beziehungen:
\begin{itemize}
\item Patterns, die zusammen verwendet werden (z.B. Composite + Iterator)
\item Patterns, die sich gegenseitig ersetzen (z.B. Prototype vs. Abstract Factory)
\item Patterns mit ähnlichen Strukturen aber unterschiedlichen Zwecken (z.B. Composite vs. Decorator)
\end{itemize}
\end{enumerate}

\section{Testen \& Softwarequalität}

\subsection{Entwicklertests}

\begin{enumerate}
\item Modultests
\begin{itemize}
\item Testen einzelner Programmeinheiten, Methoden oder Objektklassen
\item Fokus auf Fonktionalität von Objekten/Methoden
\end{itemize}
\item Komponententests
\begin{itemize}
\item Intregration mehrerer Einheiten zu zsmgesetzten Komponenten
\item Fokus auf Testen der Komponentenschnittstellen
\end{itemize}
\item Systemtests
\begin{itemize}
\item Integration von Komponenten, System als Ganzes
\item Testen der Interaktionen zw. Komponenten
\end{itemize}
\end{enumerate}
...
\begin{enumerate}
\item Aufbau: Initilisierung mit Testfällen
\item Aufruf: Aufrufen der zu testenden Methoden
\item Bewertung: Vergleich mit erwarteten Ergebnissen
\end{enumerate}

\subsubsection{Auswahl der Testfälle für Modultests}

\textbf{Ziel:} effiziente Testfälle erstellen, möglichst viele Fehler bei möglichst wenig Aufwand finden

\begin{enumerate}
\item Klassenbasierte Tests (Äquivalenzklassen)
\begin{itemize}
\item Eingaben/Ausgaben nach gemeinsamen Eigenschaften gruppieren („Äquivalenzklassen“)
\item Pro Klasse wenige repräsentative Testfälle wählen
\item Wichtig: Immer Grenzwerte testen! (z. B. min, max, genau min-1, max+1)
\item Bsp: Eingabe 10.000–99.999 $\rightarrow$ Teste 9.999, 10.000, 50.000, 99.999, 100.000
\item Blackbox-Testing: Nur Spezifikation/Dokumentation nötig, kein Code-Inhalt
\end{itemize}
\item Richtlinienbasierte Tests
\begin{itemize}
\item Einzelwert-Folgen testen (Programmierer denken oft an "mehrere Elemente")
\item Verschiedene Folgengrößen (klein/mittel/groß)  
\item Erste/mittlere/letzte Elemente einer Liste/Folge
\item Eingaben, die Fehlermeldungen auslösen
\item Pufferüberläufe provozieren
\item Ungültige Ausgaben erzwingen
\item Berechnungen mit zu groß/klein Ergebnissen
\end{itemize}
\end{enumerate}

\subsubsection{Testen von Komponenten}

\begin{itemize}
\item testen zusammengesetzte Komponenten, die aus mehreren Objekten/Modulen bestehen
\item finden nach Modultests \& vor Systemtests statt
\item Fokus: Verhalten der Komponentenschnittstellen, nicht der einzelnen Methoden im Inneren
\item Ziel: Interaktionsfehler zwischen den intern beteiligten Objekten aufdecken
\item Schnittstellentypen:
\begin{itemize}
\item Parameterschnittstellen (Daten/Funktionsreferenzen werden übergeben)
\item Schnittstellen mit gemeinsamem Speicher (mehrere Komponenten greifen auf denselben Speicher zu)
\item Weitere Schnittstellenformen (z. B. prozedurale)
\end{itemize}
\item Fehler oft durch falsche Parameter, falsche Reihenfolgen von Aufrufen oder Nebenwirkungen über geteilten Speicher
\item Testfälle gegen die äußere Schnittstelle der gesamten Komponente definiert, nicht gegen die Einzelteile
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/sst.png}
\caption{Schnittstellentests}
\end{figure}

\subsubsection{Testen von Systemen}

\begin{itemize}
\item Nach Komponententests mehrere oder alle Komponenten zu einem kompletten System integriert \& als Ganzes getestet
\item Fokus: Interaktionen zw Komponenten \& das Verhalten des Gesamtsystems aus Sicht der Anforderungen
\item Ziel: Zeigen, dass das System als Ganzes die spezifizierten Funktionen korrekt bereitstellt
\item Systemtests prüfen auch nicht-funktionale Eigenschaften (z. B. Performance, Verhalten unter Last)
\end{itemize}

\subsection{Testgetriebene Entwicklung}

\begin{itemize}
\item Indentifizierung geforderter Funktionalität
\item Test für Funktionalität schreiben \& implementieren als Automatisierten; wird ausgeführt \& protokolliert, ob bestanden oder nicht
\item Test mit allen anderen Test ausführen
\item ggf. muss Code angepasst werden
\item Alle Tests erfolgreich, weitere schreiben
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/tdd.png}
\caption{Testgetriebene Entwicklung}
\end{figure}

\begin{itemize}
\item Codeabdeckung: jedes Codesegment soll min. 1 dazugehörigen Test haben; frühe Erkennung von Fehlern
\item Regressionstest: Testsuite inkrementell entwickelt; prüft, dass neuer Code keine Fehler einführt
\item vereinfachtes Fehlerbeheben: neugeschriebenen Code überprüfen \& modifizieren
\item Systemdokumentation: Tests als eine Art von Docs
\end{itemize}

\subsection{Eigenschaften der Verlässlichkeit}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{pictures/eigver.png}
\caption{Hauptpfeiler der Verlässlichkeit}
\end{figure}

\noindent
\textbf{Weitere Eigenschaften die Verlässlichkeit beeinflussen}
\begin{itemize}
\item Reparaturfähigkeit: schnelle Wiederherstellung nach Ausfällen
\item Wartbarkeit: leichte Anpassung an neue Anforderungen
\item Fehlertoleranz: Erkennung \& Behandlung von Benutzerfehlern
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/kvk.png}
\caption{Kosten-/Verlässlichkeitskurve}
\end{figure}

\subsection{Soziotechnische Systeme}

\begin{itemize}
\item Geräteschicht: Hardwaregeräte, teilweise auch Computer
\item Betriebssystemschicht: Interaktion mit der Hardware; stellt grundlegende Funktionen für höhere Schichten bereit
\item Kommunikations- \& Datenverwaltungsschicht (Middleware): Erweitert OS-Funktionen; ermöglicht z. B. Zugriff auf entfernte Systeme \& Datenbanken
\item Anwendungsschicht: Stellt anwendungsspezifische Funktionen bereit; mehrere Anwendungen möglich
\item Geschäftsprozessschicht: Umfasst unternehmensspezifische Geschäftsprozesse, die das System nutzen
\item Unternehmensschicht: Beinhaltet strategische Prozesse, Geschäftsregeln, -richtlinien und -standards
\item Gesellschaftsschicht: Umfasst gesetzliche \& regulatorische Vorgaben für den Systemeinsatz
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{pictures/soziosys.png}
\caption{Schichten eines soziotechnischen Systems}
\end{figure}

\noindent
\textbf{Konsequenzen für Entwickler bei Fehler in einer Schicht}
\begin{itemize}
\item Ganzheitliche Systembetrachtung notwendig
\item Verstehen, wie Softwareausfälle andere Systemteile beeinflussen
\item Grenzen von Softwarefehlern definieren, um Gesamtsystemausfälle zu vermeiden
\item Sicherstellen, dass Tests auch Ausfälle in anderen Schichten erkennen
\end{itemize}

\subsubsection{Regulierung \& Einhaltung der Vorschriften}

Regierungen regeln kritische Systeme durch
\begin{itemize}
\item Standards für Betriebssicherheit \& Informationssicherheit
\item Lizenzierungsbehörden (z.B. Luftfahrt, Atomkraft)
\item Forderung nach umfangreichen Sicherheitsnachweisen
\item Teuer: Zertifizierungsdokumentation kann so teuer wie das System selbst sein
\end{itemize}
